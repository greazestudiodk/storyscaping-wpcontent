...
class OnecomSp
{
    ...
    public static function sp_admin_head($page_title, $description)
    {
        ...
        $html = '<h1 class="one-title">' . $page_title . '</h1>';

        $html .= '<div class="oc-sp-head">';
        $html .= '<h2 class="one-logo">';
        $html .= '<div class="textleft"><span>' . $description . '</span></div>';
...
//moved here to avoid recursion in case of restore plugin data
    ...
    public function oc_generate_submit_button($type = 'regular')
    {
        ...
        if ($type == 'regular') {
            return '<input class="oc-save oc-regular-btn" type="submit" disabled name="one_sp_submit" value="' . __('Save', OC_SP_TEXTDOMAIN) . '"/>';
        } elseif ($type == 'float') {
            return '<input class="oc-save oc-sp-float-btn" type="submit" disabled name="one_sp_submit" value="' . __('Save', OC_SP_TEXTDOMAIN) . '"/>';
        } else {
            return '<input class="oc-save" type="submit" disabled name="one_sp_submit" value="' . __('Save', OC_SP_TEXTDOMAIN) . '"/>';
